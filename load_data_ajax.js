var ikona = "";
var query = "SELECT *, TIMESTAMPDIFF(minute, NOW(),data_wygasniecia)as'zostalo' FROM obrazki join uzytkownicy using(id_uzytkownika) join obrazek_kategorie USING(id_obrazka) join kategorie USING(id_kategorii) order by zostalo";
//console.log(query);
document.onload = new function () {
		...
		success: function (data) {
			...
			for (var i = 0; i < tab.length-1; i++) 
			{
				...
				}else
				{
					...
					ikona = ikona + '</div><img src="'+tab[i].obrazek+'"><div class="szot__title">'+tab[i].tytul+'</div><div class="szot__desc">'+tab[i].opis+'</div><div class="szot__like_plus horizontal-container"><form><input type="submit" value="" class="plus_btn"></form>22</div><div class="szot__time">Minuty do wygaśnięcia: '+ tab[i].zostalo+'</div></div>';
	...
	$(document).ready(function() {
		$("#author_search").click(function() {
			...
			query = "SELECT *,TIMESTAMPDIFF(minute, NOW(),data_wygasniecia)as'zostalo' FROM obrazki join uzytkownicy using(id_uzytkownika) join obrazek_kategorie USING(id_obrazka) join kategorie USING(id_kategorii) where login = '"+name+"' order by zostalo";
			console.log(query);
			show(query);
		});
